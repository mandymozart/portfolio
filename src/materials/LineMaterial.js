...
async function getTexture(name) {
  ...
  if (!papers[name].promise) {
    papers[name].promise = new Promise((resolve, reject) => {
      loader.load(`/textures/${papers[name].file}`, (res) => {
...
class LineMaterial extends MeshStandardMaterial {
  constructor(parameters) {
    ...
    this.uniforms = {
      ...
      paperTexture: { value: this.parameters?.paperTexture ?? null },
    ...
    this.onBeforeCompile = (shader, renderer) => {
        console.log('onBeforeCompile',shader,renderer,this.uniforms.paperTexture.value);
